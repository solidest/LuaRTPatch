...
static void incstepRT (lua_State *L, global_State *g) {

  //只要时间允许，gc频率比通常情况更频繁（8倍）
  int stepmul = (getgcparam(g->gcstepmul) | 1);  /* avoid division by 0 */
  ...
  //int tlimit = get_gctick()/3;
  //开始时间
  __u64 tstart = rdtsc();
  __u64 tnow = tstart;
  //时间片2/3
  //__u64 tHalf = tnow + tlimit*2;
  //时间片结束点
  __u64 tend = tnow + get_gctick();
  ...
  do {  /* repeat until pause or enough "credit" (negative debt) */
    ...
    if(tnow>tend)
    {
        RTGC_TIMEOUT(tend, tnow);
        break;
    }

    //如果时间过2/3不再继续原子操作
    // if(g->gcstate == GCSenteratomic && tnow>tHalf)
    // {
    //     RTGC_ATIMEOUT(); 
    //     //break;
    // }
